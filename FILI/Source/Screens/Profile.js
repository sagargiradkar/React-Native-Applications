...
const Profile = (props) => {
  ...
  const onLogOut = async () => {
    try {
      ...
      fetch("http://192.168.143.131:9005/api/v1/users/logout", requestOptions)
        .then(async (response) => {
      ...
      <View style={styles(colors).innercontainer}>
        ...
        <View>
          <ProfileElement
            icon="user-tie"
            text="Profile"
            onPress={onProfile}></ProfileElement>
          <ProfileElement
            icon="gift"
            text="F.A.Q."
            onPress={() => {
          ...
          <ProfileElement
            icon="bookmark"
            text="Interests"
            onPress={onInterest}></ProfileElement>
          <ProfileElement
            icon="fire-alt"
            text="Thundering"
            onPress={onStreak}></ProfileElement>
          <ProfileElement
            icon="sign-out-alt"
            text="Logout"
            logout={true}
            onPress={onLogOut}></ProfileElement>
        </View>
