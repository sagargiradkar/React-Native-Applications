...
const Question = ({ ...props }) => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.143.131:8085/api/v1/quizzes/user/userQuiz",
        requestOptions
      );
  ...
  const handleScroll = (event) => {
    if (event && event.nativeEvent) {
      ...
      setCurrentIndex(newIndex); // Update currentIndex based on scrolling
    }
  };
  ...
  return (
    <AlertNotificationRoot>
      ...
      <Text style={styles(colors).questions}>⏩ Question No.{currentIndex + 1}</Text>
      <ScrollView
