  ...
  const fetchData = async () => {
    try {
      ...
      const [streakResponse, userResponse] = await Promise.all([
        fetch("http://192.168.143.131:8085/api/v1/streakommits", requestOptions),
        fetch("http://192.168.143.131:9005/api/v1/users/profile", requestOptions),
      ]);
  ...
  const setProfile = async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.143.131:9005/api/v1/users/profile",
        requestOptions
      );
