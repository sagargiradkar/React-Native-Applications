...
const Post = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        "http://192.168.143.131:8086/api/v1/topics/users/getAllTopics",
        {
          method: "GET",
  ...
  const fetchCommentsForPost = useCallback(async (postId) => {
    try {
      const authToken = await AsyncStorage.getItem("authToken");
      const response = await fetch(
        `http://192.168.143.131:8086/api/v1/comments/topicComment/${postId}`,
        {
          method: "GET",
          headers: { Authorization: authToken },
        }
  ...
  const likePostOrComment = async (postId, commentId) => {
    try {
      ...
      let likeUrl;
      if (commentId) {
        likeUrl = `http://192.168.143.131:8086/api/v1/comments/commentLikeReaction/${commentId}`;
      } else {
        likeUrl = `http://192.168.143.131:8086/api/v1/topics/userTopicLikeReaction/${postId}`;
      }
  ...
  const postDelete = async (postId) => {
    ...
    try {
      ...
      const deleteUrl = `http://192.168.143.131:8086/api/v1/topics/${postId}`;
  ...
  const saveModifiedPost = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.143.131:8086/api/v1/topics/${modifiedPostId}`,
        requestOptions
      );
      if (response.ok) {
        const updatedCommunityData = communityData.map((post) => {
